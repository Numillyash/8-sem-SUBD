...
if [[ "$need_setup" == "yes" ]]; then
  echo "=== Running extended setup because functions/tables are missing ==="
  "${PSQL[@]}" -f sql/07a_extended_setup_only.sql
fi

if [[ "${DENSE_RESET:-0}" == "1" ]]; then
  echo "=== Backing up old extended DB tables if backup does not exist ==="

  psql_exec "
DO \$\$
BEGIN
    IF to_regclass('lab4.extended_all_sizes_measurements_backup_10_to_50m') IS NULL
       AND to_regclass('lab4.extended_all_sizes_measurements') IS NOT NULL THEN
        EXECUTE 'CREATE TABLE lab4.extended_all_sizes_measurements_backup_10_to_50m AS TABLE lab4.extended_all_sizes_measurements';
    END IF;

    IF to_regclass('lab4.extended_all_sizes_sizes_backup_10_to_50m') IS NULL
       AND to_regclass('lab4.extended_all_sizes_sizes') IS NOT NULL THEN
        EXECUTE 'CREATE TABLE lab4.extended_all_sizes_sizes_backup_10_to_50m AS TABLE lab4.extended_all_sizes_sizes';
    END IF;

    IF to_regclass('lab4.extended_update_trigger_audit_backup_10_to_50m') IS NULL
       AND to_regclass('lab4.extended_update_trigger_audit') IS NOT NULL THEN
        EXECUTE 'CREATE TABLE lab4.extended_update_trigger_audit_backup_10_to_50m AS TABLE lab4.extended_update_trigger_audit';
    END IF;
END
\$\$;
"

  echo "=== Resetting current extended tables for dense under 1M run ==="

  psql_exec "
TRUNCATE TABLE
    lab4.extended_all_sizes_measurements,
    lab4.extended_all_sizes_sizes,
    lab4.extended_update_trigger_audit
RESTART IDENTITY;
"
fi

sizes=(10 25 50 100 250 500 1000 2500 5000 10000 25000 50000 100000 250000 500000)
